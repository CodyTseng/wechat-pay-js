...
export type CreateCombineTrasactionOptions = {
...
export type CreateCombineTransactionJsapiResult = {
  prepay_id: string; // 预支付交易会话标识
};

export type CreateCombineTransactionAppResult = {
  prepay_id: string; // 预支付交易会话标识
};

export type CreateCombineTransactionNativeResult = {
  code_url: string; // 二维码链接
};

export type CombineTransactionDetails = {
  ...
  sub_orders: {
    mchid: string; // 子单商户号
    trade_type: 'NATIVE' | 'JSAPI' | 'APP' | 'MWEB'; // 交易类型
    trade_state:
      | 'SUCCESS'
      | 'REFUND'
      | 'NOTPAY'
    ...
    promotion_detail?: {
      ...
      name?: string; // 优惠名称
      scope?: 'GLOBAL' | 'SINGLE'; // 优惠范围
      type?: 'CASH' | 'NOCASH'; // 优惠类型
      amount: number; // 优惠券金额
      stock_id?: string; // 活动ID
...
export type CloseCombineTransactionOptions = {
  // 子单信息
  sub_orders: {
    mchid: string; // 子单商户号
    out_trade_no: string; // 子单商户订单号
...
export type QueryGertificatesResult = {
  data: {
    effective_time: string;
    encrypt_certificate: {
      algorithm: string;
